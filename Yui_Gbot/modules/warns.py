...
from Yui_Gbot import dispatcher, BAN_STICKER, WHITELIST_USERS, TIGER_USERS
from Yui_Gbot.modules.disable import DisableAbleCommandHandler
from Yui_Gbot.modules.helper_funcs.chat_status import (is_user_admin, bot_admin, user_admin_no_reply, user_admin,
                                                     can_restrict)
from Yui_Gbot.modules.helper_funcs.extraction import extract_text, extract_user_and_text, extract_user
...
def __stats__():
    return (f"{sql.num_warns()}<b>» Warns, Across</b>{sql.num_warn_chats()}<b>Chats</b>\n\n"
            f"{sql.num_warn_filters()}<b>» Warn Filters, Across </b>{sql.num_warn_filter_chats()}<b>Chats</b>")
